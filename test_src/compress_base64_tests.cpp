...
TEST(CompressBase64, HelloWorld) {
  std::string input(hello_world);

  std::vector<uint16_t> result = {66, 73, 85,  119, 78,  109, 68, 50,
                                  65, 69, 68,  113, 107, 67,  99, 119,
                                  66, 78, 111, 69,  73,  78,  65, 61};

  EXPECT_TRUE(compare_u16(LZStringOptions::BASE64, input, result));
}

TEST(CompressBase64, AllASCII) {
  ...
  std::vector<uint16_t> result = {
      ...
      103, 65,  43,  65,  76,  52,  65,  102, 107, 65,  61};

  EXPECT_TRUE(compare_u16(LZStringOptions::BASE64, input, result));
}

TEST(CompressBase64, JSON) {
  ...
  std::vector<uint16_t> result = {
      ...
      77,  72,  73,  81,  121, 82,  75,  112, 109, 75,  112, 65,  65,  61,  61};

  EXPECT_TRUE(compare_u16(LZStringOptions::BASE64, input, result));
}

TEST(CompareBase64, JSONFloat) {
  ...
  std::vector<uint16_t> result = {
      ...
      73,  55,  101, 108, 81,  54,  79,  118, 109, 103, 65,  61};
