...
std::string
get_text_file_content(std::string_view filename)
{
...
std::u16string
get_utf16_text_file_content(std::string_view filename)
{
...
std::vector<uint8_t>
get_bin_file_content(std::string_view filename)
{
...
std::vector<uint16_t>
get_utf16_bin_file_content(std::string_view filename)
{
...
bool
get_decompress_test_result(std::string_view test_name, LZStringOptions option)
{
...
get_compressed(std::string_view value, LZStringOptions option)
{
  switch (option) {
    ...
    case LZStringOptions::URI:
      return pxd::lz_string::compressEncodedURI(
            ...
            const std::vector<uint16_t>& result)
{
  ...
  return std::equal(
    compress_result.begin(), compress_result.end(), result.begin());
}

bool
compare_u16(std::string_view input, LZStringOptions option)
{
